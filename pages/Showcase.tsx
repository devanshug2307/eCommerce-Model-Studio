        ...
        {openBA && (
          <div className="fixed inset-0 z-50">
            ...
            <div className="absolute inset-0 flex items-center justify-center p-4">
              <div className="bg-gray-900 border border-gray-700 rounded-xl max-w-4xl w-full p-4">
                ...
                {openBA.input_public_url ? (
                  <div className="rounded overflow-hidden">
                    <iframe title="before-after" style={{width:'100%', height:'65vh', border:'0'}} srcDoc={`<!DOCTYPE html><html><head><meta charset='utf-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/><style>body{margin:0;background:#0b0b0b}</style></head><body><div id='root'></div><script>const root=document.getElementById('root');const wrap=document.createElement('div');wrap.style.position='relative';wrap.style.maxWidth='100%';wrap.style.height='65vh';wrap.style.overflow='hidden';const before=document.createElement('img');before.src='${openBA.input_public_url}';before.style.width='100%';before.style.height='100%';before.style.objectFit='contain';const after=document.createElement('img');after.src='${openBA.public_url}';after.style.width='100%';after.style.height='100%';after.style.objectFit='contain';after.style.position='absolute';after.style.top='0';after.style.left='0';const slider=document.createElement('input');slider.type='range';slider.min='0';slider.max='100';slider.value='50';slider.style.position='absolute';slider.style.bottom='8px';slider.style.left='8px';slider.style.right='8px';slider.style.width='calc(100% - 16px)';slider.oninput=()=>{after.style.clipPath='inset(0 '+(100-slider.value)+'% 0 0)'};after.style.clipPath='inset(0 50% 0 0)';wrap.appendChild(before);wrap.appendChild(after);wrap.appendChild(slider);root.appendChild(wrap);</script></body></html>`} />
                  </div>
                ) : null}
              </div>
            </div>
